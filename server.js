...
// ID Программ и Кампаний из документации
...
const allowedOrigins = [
  ...
  'https://app-quqixs-projects.vercel.app'    // Твой второй технический адрес
];
...
app.post('/api/generate-flight-deeplink', (req, res) => {
    ...
    const deeplinkParams = { marker: YOUR_PARTNER_MARKER, trs: TRS_VALUE, p: P_VALUE_AVIASALES, u: encodedTargetUrl, campaign_id: CAMPAIGN_ID_AVIASALES };
...
app.post('/api/generate-hotel-deeplink', (req, res) => {
    ...
    const encodedTargetUrl = encodeURIComponent(targetUrl);
    const affiliateDeeplinkParams = { marker: YOUR_PARTNER_MARKER, trs: TRS_VALUE, p: P_VALUE_HOTELLOOK, u: encodedTargetUrl, campaign_id: CAMPAIGN_ID_HOTELLOOK };
    const affiliateDeeplink = `https://tp.media/r?${querystring.stringify(affiliateDeeplinkParams)}`;
    res.json({ success: true, deeplink: affiliateDeeplink });
});

// 11. Маршрут для получения списка самых дешевых туров от Travelata
...
// 12. Маршрут для генерации deeplink для Туров
app.post('/api/generate-tour-deeplink', (req, res) => {
    ...
    const affiliateDeeplinkParams = {
        ...
        trs: TRS_VALUE, 
        p: P_VALUE_TRAVELATA, // ID программы для Travelata
        u: encodedTargetUrl, 
        campaign_id: CAMPAIGN_ID_TRAVELATA // ID кампании для Travelata
    };
    const affiliateDeeplink = `https://tp.media/r?${querystring.stringify(affiliateDeeplinkParams)}`;
    res.json({ success: true, deeplink: affiliateDeeplink });
});
